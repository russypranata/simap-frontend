...
import { useMutamayizinTutorAttendance } from '../hooks/useMutamayizinEkskul';
...
import type { TutorAttendanceItem } from '../services/mutamayizinService';
...
const getSessionBadgeClass = (count: number): string => {
...
export const TutorRecap: React.FC = () => {
    ...
    const selectedYearId =
        academicYearFilter || (activeYear ? String(activeYear.id) : '');
    ...
    const totalTutors = new Set(
        (tutorAttendance ?? []).map((t) => t.tutorId)
    ).size;
    const totalSessions = (tutorAttendance ?? []).reduce(
        (sum, t) => sum + t.sessionCount,
        0
    );
                    ...
                    <div className="flex items-center gap-3 relative z-10">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-white">
                                Statistik Keaktifan Tutor
                            </h2>
                            <p className="text-blue-100 text-sm">
                                Ringkasan performa presensi tutor
                                {selectedYearName
                                    ? ` TA ${selectedYearName}`
                                    : ''}
                            </p>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead className="bg-muted/50">
                                    <tr>
                                        <th className="text-center p-4 font-medium text-sm w-12">
                                            No
                                        </th>
                                        <th className="text-left p-4 font-medium text-sm min-w-[180px]">
                                            Nama Tutor
                                        </th>
                                        <th className="text-left p-4 font-medium text-sm min-w-[160px]">
                                            Nama Ekskul
                                        </th>
                                ...
                                <tbody>
                                    {isLoading ? (
                                        ...
                                        paginatedData.map((item, index) => (
                                            <tr
                                                key={`${item.tutorId}-${item.ekskulName}`}
