...
import type { AcademicYear, ExtracurricularItem } from "../services/mutamayizinService";
...
const DetailModal: React.FC<DetailModalProps> = ({ tutorId, open, onClose }) => {
    const { data: detail, isLoading, error } = useMutamayizinExtracurricularDetail(tutorId ?? 0);
                ...
                {detail && !isLoading && (
                    <div className="space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-4 p-4 bg-blue-50 rounded-lg border border-blue-100">
                            <div>
                                <p className="text-xs text-muted-foreground">Nama Ekskul</p>
                                <p className="font-semibold text-slate-900">{detail.name}</p>
                            </div>
                            <div>
                                <p className="text-xs text-muted-foreground">Nama Tutor</p>
                                <p className="font-semibold text-slate-900">{detail.tutorName}</p>
                            </div>
                            <div>
                                <p className="text-xs text-muted-foreground">Jumlah Anggota</p>
                                <p className="font-semibold text-slate-900">{detail.memberCount} siswa</p>
                            </div>
                            <div>
                                <p className="text-xs text-muted-foreground">Kehadiran Rata-rata</p>
                                <p className="font-semibold text-slate-900">{detail.attendanceRate}%</p>
                            </div>
                        </div>
                                        ...
                                        <tbody>
                                            {detail.members.map((member, idx) => (
                                                <tr key={member.id} className="border-t hover:bg-muted/20">
                                                    ...
                                                    <td className="p-3 font-mono text-xs">{member.nis}</td>
                                                    <td className="p-3 font-medium">{member.name}</td>
                                                    <td className="p-3">{member.class}</td>
...
export const MutamayizinTutors: React.FC = () => {
    ...
    const { data: extracurriculars, isLoading, error, refetch } = useMutamayizinExtracurriculars(params);
    ...
    const avgAttendance = extracurriculars && extracurriculars.length > 0
        ? Math.round(extracurriculars.reduce((sum, e) => sum + e.attendanceRate, 0) / extracurriculars.length)
        : 0;
    const totalMembers = extracurriculars?.reduce((sum, e) => sum + e.memberCount, 0) ?? 0;
            ...
            <Card className="overflow-hidden p-0">
                <div className="bg-blue-800 p-4 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 relative z-10">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-white">Statistik Tutor Ekskul</h2>
                ...
                <CardContent className="p-0">
                    <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x">
                        <div className="p-2.5 text-center">
                            ...
                            <p className="text-xs font-medium text-muted-foreground mt-0.5">Total Ekskul</p>
                        </div>
                        <div className="p-2.5 text-center">
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <CardTitle className="text-lg font-semibold">Daftar Tutor Ekskul</CardTitle>
                                    <CardDescription>Data lengkap tutor beserta ekskul yang diampu</CardDescription>
                                </div>
                            </div>
                            ...
                            <table className="w-full">
                                <thead className="bg-muted/50">
                                    <tr>
                                        ...
                                        <th className="text-left p-4 font-medium text-sm min-w-48">Nama Tutor</th>
                                        <th className="text-left p-4 font-medium text-sm min-w-36">Nama Ekskul</th>
                                        <th className="text-center p-4 font-medium text-sm w-36">Jumlah Anggota</th>
                                        <th className="text-center p-4 font-medium text-sm w-44">Kehadiran Rata-rata</th>
                                        <th className="text-left p-4 font-medium text-sm w-40">Tanggal Bergabung</th>
                                        <th className="text-center p-4 font-medium text-sm w-28">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {isLoading ? (
                                            ...
                                            <tr key={item.id} className="border-b hover:bg-muted/30 transition-colors">
                                                ...
                                                <td className="p-4">
                                                    <div className="flex flex-col">
                                                        <span className="font-medium text-gray-900">{item.tutorName}</span>
                                                        <span className="text-xs text-muted-foreground font-mono">{item.tutorNip}</span>
                                                    </div>
                                                </td>
                                                <td className="p-4">
                                                ...
                                                <td className="p-4 text-center">
                                                    <span className="font-semibold text-slate-800">{item.memberCount}</span>
                                                    <span className="text-xs text-muted-foreground ml-1">siswa</span>
                                                </td>
                                                <td className="p-4 text-center">
                                                    <Badge className={getAttendanceBadgeClass(item.attendanceRate)}>
