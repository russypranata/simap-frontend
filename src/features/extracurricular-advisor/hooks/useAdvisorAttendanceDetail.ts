...
export const useAdvisorAttendanceDetail = (id: number): UseAdvisorAttendanceDetailReturn => {
    ...
    // Reset page when filters change — done via setters instead of useEffect
    const setSearchTerm = (s: string) => { setSearchTermRaw(s); setCurrentPage(1); };
    const setStatusFilter = (s: string) => { setStatusFilterRaw(s); setCurrentPage(1); };
    const setClassFilter = (c: string) => { setClassFilterRaw(c); setCurrentPage(1); };
