...
export const getStudentDailyAttendance = async (
    ...
    const response = await fetch(`${STUDENT_API_URL}/attendance/daily?${params}`, { headers: getAuthHeaders() });
    if (!response.ok) await handleApiError(response);
    const result = await response.json();

    const apiMap: Record<string, DailyAttendanceRecord> = {};
    for (const item of result.data ?? []) {
        apiMap[item.date] = { date: item.date, status: statusMap[item.status] ?? 'belum_dicatat', notes: item.notes };
    }
    ...
    return { records, studentName: result.studentName ?? '', studentClass: result.studentClass ?? '' };
};
