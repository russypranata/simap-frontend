...
import { generateIdCardPDF } from '../utils/generateIdCardPDF';

export const StudentProfile: React.FC = () => {
    const router = useRouter();
    const [isPhotoOpen, setIsPhotoOpen] = React.useState(false);
    const [isGeneratingPDF, setIsGeneratingPDF] = React.useState(false);
    const [profilePictureDataUrl, setProfilePictureDataUrl] = React.useState<
        string | null
    >(null);
    ...
    useEffect(() => {
        ...
        const fetchViaProxy = (url: string) =>
            fetch(`/api/image-proxy?url=${encodeURIComponent(url)}`).then((r) =>
                r.blob(),
            );

        fetchViaProxy(profileData.profilePicture)
            .then(blobToDataUrl)
            .then(setProfilePictureDataUrl)
            .catch(() => {
                setProfilePictureDataUrl(profileData.profilePicture ?? null);
            });
    ...
    const handleDownloadPDF = async () => {
        if (!frontCardRef.current || !backCardRef.current || !profileData) {
            toast.error('Gagal memuat elemen kartu pelajar');
            return;
        }
        setIsGeneratingPDF(true);
        try {
            const barcodeDataUrl = barcodeRef.current
                ? barcodeRef.current.toDataURL('image/png')
                : undefined;
            await generateIdCardPDF(
                frontCardRef.current,
                backCardRef.current,
                `kartu-pelajar-${profileData.nis}.pdf`,
                barcodeDataUrl,
            );
            toast.success('Kartu pelajar berhasil diunduh!');
        } catch (e) {
            console.error('PDF generation error:', e);
            toast.error('Gagal mengunduh kartu pelajar');
        } finally {
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        Lihat informasi pribadi dan data akademik Anda
                    </p>
                </div>
            </div>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground mt-0.5 font-normal">
                                    Pratinjau kartu identitas Anda
                                </p>
                            </div>
                        </div>
                        <Button
                    ...
                    <div className="mb-6 p-4 rounded-lg bg-blue-50/50 border border-blue-100 flex gap-3 text-blue-900/80">
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-xs sm:text-sm leading-relaxed text-blue-800/80">
                                ...
                                dan barcode yang tajam sesuai standar kartu
                                pelajar.
                            </p>
                        </div>
                    </div>
