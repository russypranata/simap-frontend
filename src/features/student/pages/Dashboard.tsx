...
import {
    Calendar, GraduationCap, CheckCircle, Trophy, ChevronRight,
    TrendingUp, AlertTriangle, ClipboardList, Timer, Moon, Activity, Clock,
} from "lucide-react";
...
export const StudentDashboard: React.FC = () => {
    ...
    if (error || !data) return <ErrorState error={error ?? "Terjadi kesalahan"} onRetry={refetch} />;
    ...
    return (
        <div className="space-y-6">
            ...
            {hasWarning && (
                <div className="flex items-center gap-3 px-4 py-3 rounded-lg bg-amber-50 border border-amber-200">
                    ...
                    <p className="text-sm text-amber-800 flex-1">
                        ...
                        {stats.attendanceRate < 80 && `Tingkat kehadiran kamu rendah (${stats.attendanceRate}%). `}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
                ...
                <StatCard title="Kehadiran" value={`${stats.attendanceRate}%`} subtitle="Tahun Ajaran Ini" icon={CheckCircle} color="green" href="/student/attendance/daily"
                    badgeColor={stats.attendanceRate >= 80 ? "green" : stats.attendanceRate >= 75 ? "amber" : "red"} />
                <StatCard title="Pagi" value={stats.lateCount} subtitle="Keterlambatan Pagi" icon={Timer} color="amber" href="/student/attendance/morning"
                ...
                <StatCard title="Prestasi" value={stats.achievementsCount} subtitle="Penghargaan" icon={Trophy} color="purple" href="/student/achievements"
                    badgeColor="green" />
                <StatCard title="Perilaku" value={stats.violationCount} subtitle="Pelanggaran Aktif" icon={ClipboardList} color="rose" href="/student/behavior"
                    badgeColor={stats.violationCount === 0 ? "green" : stats.violationCount <= 3 ? "amber" : "red"} />
            </div>

            {/* Row 1: Schedule + Subject Performance */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Today's Schedule */}
                            ...
                            <div className="space-y-2">
                                {todaySchedule.map((item) => (
                                    <div key={item.id} className={cn(
                                        ...
                                        item.isOngoing ? "bg-blue-100/60 border-blue-200" : "bg-blue-50/50 border-blue-100"
                                    )}>
                                        <div className="flex items-center gap-2.5 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <p className="text-[11px] text-slate-500 mt-0.5">{data.studentClass}</p>
                                            </div>
                                        </div>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <CardDescription>Tertinggi & perlu perhatian semester ini</CardDescription>
                                </div>
                            </div>
