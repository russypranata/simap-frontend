...
import { getScoreColor, getGradeColor } from "@/features/parent/components/grades/helpers";
...
export const StudentGrades: React.FC = () => {
    ...
    const extracurriculars: Extracurricular[] = (ekskulQuery.data?.extracurriculars ?? []).map(e => ({
        name:        e.name,
        type:        "Pilihan" as const,
        score:       e.attendanceRate >= 90 ? "A" : e.attendanceRate >= 75 ? "B" : "C",
        predicate:   e.attendanceRate >= 90 ? "Sangat Baik" : e.attendanceRate >= 75 ? "Baik" : "Cukup",
        ...
        <div className="space-y-6">
            ...
            <GradesFilterBadges
                ...
                onClearSemester={() => {
                    ...
                    handleApplyFilter(
                        selectedYearId,
                        firstCompleted?.id ?? academicYears[0]?.semesters.find(s => s.status === "completed")?.id ?? "ganjil"
                    );
                }}
            />
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 items-stretch print:hidden">
                    ...
                    <StatCard title="Peringkat Kelas" value={stats.currentRank > 0 ? `#${stats.currentRank}` : "-"} subtitle={stats.totalStudents > 0 ? `dari ${stats.totalStudents} siswa` : "belum tersedia"} icon={Trophy} color="purple" />
                </div>
                            ...
                            <CardContent className="p-0">
                                {grades.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center py-16 text-center px-4">
                                        ...
                                        <p className="text-sm font-medium text-slate-600">Belum ada nilai untuk semester ini</p>
                                        <p className="text-xs text-slate-400 mt-1">Nilai akan muncul setelah guru menginput penilaian</p>
                                    </div>
                                ) : (
                                    <>
                            ...
                            <CardHeader className="pb-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <CardDescription className="text-sm text-slate-600">Perkembangan rata-rata nilai dari semua semester yang tercatat</CardDescription>
                                    </div>
                                </div>
                            </CardHeader>
                            <CardContent>
