...
const Skeleton_ = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <div className="rounded-xl border border-slate-200 overflow-hidden bg-white shadow-sm">
            ...
            {Array.from({ length: 7 }).map((_, i) => <Skeleton key={i} className="h-16 w-full rounded-none border-b border-slate-100" />)}
        </div>
    </div>
);

export const StudentPrayerAttendance: React.FC = () => {
    const { records, selectedMonth, selectedYear, isLoading, isFetching, error, isCurrentMonth, handlePrevMonth, handleNextMonth, handleCurrentMonth, refetch } = useStudentPrayerAttendance();
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-between items-center">
                <div className="flex items-center gap-1.5 bg-white rounded-lg border shadow-sm px-3 py-1.5">
                    <button onClick={handlePrevMonth} className="h-7 w-7 rounded-lg flex items-center justify-center bg-blue-800 text-white hover:bg-blue-900 transition-colors">
                        <ChevronLeft className="h-4 w-4" />
                    </button>
                    <div className="flex items-center justify-center gap-2 min-w-[160px] cursor-pointer group" onClick={handleCurrentMonth} title="Kembali ke bulan ini">
                        <CalendarIcon className="h-3.5 w-3.5 text-slate-400 group-hover:text-primary transition-colors" />
                        <span className="text-sm font-semibold text-slate-900 group-hover:text-primary transition-colors">{MONTHS[selectedMonth]} {selectedYear}</span>
                    </div>
                    <button onClick={handleNextMonth} disabled={isCurrentMonth} className={cn("h-7 w-7 rounded-lg flex items-center justify-center transition-colors", !isCurrentMonth ? "bg-blue-800 text-white hover:bg-blue-900" : "bg-slate-100 text-slate-300 cursor-not-allowed")}>
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                    {[
                        { label: "Hadir Tepat Waktu", color: "bg-emerald-50 text-emerald-700 border-emerald-200", icon: CheckCircle },
                        { label: "Masbuk", color: "bg-blue-50 text-blue-700 border-blue-200", icon: Clock },
                        { label: "Haid", color: "bg-pink-50 text-pink-700 border-pink-200", icon: CheckCircle },
                        { label: "Tanpa Keterangan", color: "bg-red-50 text-red-700 border-red-200", icon: XCircle },
                    ].map(({ label, color, icon: Icon }) => (
                        <div key={label} className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium shadow-sm", color)}>
            ...
            <div className="rounded-xl border border-slate-200 overflow-hidden relative bg-white shadow-sm">
                ...
                {records.length === 0 ? (
                    <div className="flex flex-col items-center justify-center text-center py-20 px-4">
                        ...
                        <p className="text-sm text-slate-500 max-w-md mt-1">Tidak ada riwayat sholat yang tercatat untuk periode yang dipilih.</p>
                    </div>
                ) : (
                            ...
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-200">
                                    ...
                                    {PRAYER_COLUMNS.map((col, idx) => (
                                        <th key={col.key} className={cn("text-center p-4 font-semibold text-xs text-slate-600 uppercase tracking-wider border-r border-slate-200", idx % 2 === 0 && "bg-slate-100/50")}>
                                            {col.label}
                                        </th>
                                    ))}
                                </tr>
                                            ...
                                            return (
                                                <td key={key} className={cn("p-2 border-r border-slate-200 text-center align-middle", idx % 2 === 0 && "bg-slate-50/30")}>
                                                    <div className="flex flex-col items-center justify-center gap-1.5">
                                                        <div className={cn("p-1.5 rounded-full shadow-sm border", config.color)}><Icon className="h-4 w-4" /></div>
