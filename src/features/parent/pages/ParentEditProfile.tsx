...
export const ParentEditProfile: React.FC = () => {
    ...
    const handleSave = async (data: ParentProfileData, file: File | null) => {
        ...
        try {
            ...
            if (file) {
                const avatarUrl = await uploadParentAvatar(file);
                // Update cache dengan avatar baru
                queryClient.setQueryData(["parent-profile"], {
                    ...updated,
                    profilePicture: avatarUrl,
                });
            } else {
                // Update cache langsung dengan data terbaru dari server
                queryClient.setQueryData(["parent-profile"], updated);
            }
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-start gap-3">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Kelola data diri, kontak, dan pengaturan sandi Anda</p>
                </div>
            </div>
            ...
            <ParentProfileForm
                ...
                onSave={handleSave}
                onCancel={() => router.push("/parent/profile")}
                isLoading={isLoading}
            />
