...
import { StudentRecap, StudentRecapList } from "./subject-recap/StudentRecapList";
import { StudentSubjectDetail } from "./subject-recap/StudentSubjectDetail";

export const SubjectAttendanceRecap = () => {
    const [periodFilter, setPeriodFilter] = useState("harian");
    const [searchTerm, setSearchTerm] = useState("");
    const [selectedStudent, setSelectedStudent] = useState<StudentRecap | null>(null);
    ...
    // Mock Data Generator
    const students: StudentRecap[] = useMemo(() => {
        return Array.from({ length: 45 }).map((_, i) => {
            const subjects = ["Matematika", "Fisika", "Kimia", "Biologi", "B. Indo", "B. Inggris"];
            const ringkasanPresensi = subjects.map(subj => ({
                mapel: subj,
                persentase: Math.floor(Math.random() * 30) + 70 // 70-100%
            })).sort((a, b) => a.persentase - b.persentase); // Show lowest first for attention

            const rataRataKehadiran = Math.round(ringkasanPresensi.reduce((acc, curr) => acc + curr.persentase, 0) / ringkasanPresensi.length);
    ...
    const handleSelectStudent = (student: StudentRecap) => {
    ...
    const getDescription = (filter: string) => {
        ...
        switch (filter) {
            case "harian":
                return `Menampilkan data presensi mata pelajaran untuk kelas XII A pada hari ${today.toLocaleDateString('id-ID', options)}.`;
            case "mingguan":
                return "Menampilkan rekap presensi mata pelajaran mingguan untuk kelas XII A. Data mencakup persentase kehadiran per siswa dalam minggu ini.";
            case "bulanan":
                return `Menampilkan rekap presensi mata pelajaran bulanan untuk kelas XII A periode ${today.toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })}.`;
            case "semester":
                return "Menampilkan rekap presensi mata pelajaran untuk kelas XII A pada Semester Ganjil Tahun Ajaran 2024/2025.";
            default:
                return "Rekap presensi mapel untuk seluruh siswa dalam kelas.";
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col space-y-1">
                <h3 className="text-2xl font-bold tracking-tight">Presensi Mata Pelajaran</h3>
                <p className="text-muted-foreground">
                    Rekap presensi mapel untuk seluruh siswa dalam kelas.
                </p>
            </div>
