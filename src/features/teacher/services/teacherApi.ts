...
const mockAxios = {
  get: async <T>(url: string): Promise<{ data: T }> => {
    ...
    switch (url) {
      ...
      case '/teacher/journals':
        return { data: mockTeachingJournals };
      case '/teacher/grades':
  ...
  post: async <T>(url: string, data: any): Promise<{ data: T }> => {
    ...
    switch (url) {
      case '/teacher/attendance':
        return { data: { success: true, message: 'Absensi berhasil disimpan' } };
      case '/teacher/journals':
        return { data: { success: true, message: 'Jurnal mengajar berhasil disimpan' } };
      case '/teacher/grades':
        return { data: { success: true, message: 'Nilai berhasil disimpan' } };
      case '/teacher/documents':
        return { data: { success: true, message: 'Dokumen berhasil diupload' } };
      default:
        throw new Error(`API endpoint not found: ${url}`);
    }
  },
  ...
  put: async <T>(url: string, data: any): Promise<{ data: T }> => {
    await new Promise(resolve => setTimeout(resolve, 500));
    return { data: { success: true, message: 'Data berhasil diperbarui' } };
  },

  delete: async <T>(url: string): Promise<{ data: T }> => {
    await new Promise(resolve => setTimeout(resolve, 500));
    return { data: { success: true, message: 'Data berhasil dihapus' } };
  },
};
...
export const teacherApi = {
  ...
  getStudents: async (classId?: string): Promise<Student[]> => {
    ...
    if (classId) {
      return response.data.filter(student => student.class === classId);
    }
    return response.data;
  },
  ...
  getAttendanceRecords: async (classId?: string, date?: string): Promise<AttendanceRecord[]> => {
    ...
    if (classId) {
      filtered = filtered.filter(record => record.class === classId);
    }
